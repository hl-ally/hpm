...
#define FREQ_PRESET1_PLL0_CLK1 (600000000UL)
#define FREQ_PRESET1_PLL0_CLK2 (400000000UL)
#define FREQ_PRESET1_PLL1_CLK0 (800000000UL)
#define FREQ_PRESET1_PLL1_CLK1 (666000000UL)
#define FREQ_PRESET1_PLL1_CLK2 (500000000UL)
#define FREQ_PRESET1_PLL1_CLK3 (266000000UL)
...
static EWDG_Type *const s_wdgs[] = { HPM_WDG0, HPM_WDG1};
...
{
    uint32_t freq_in_hz;
    if (EWDG_CTRL0_CLK_SEL_GET(HPM_PWDG->CTRL0) == 0) {
...
clk_src_t clock_get_source(clock_name_t clock_name)
{
    ...
    case CLK_SRC_GROUP_WDG:
        if (node_or_instance < WDG_INSTANCE_NUM) {
            ...
            clk_src_index = (EWDG_CTRL0_CLK_SEL_GET(s_wdgs[node_or_instance]->CTRL0) == 0);
        }
        break;
    case CLK_SRC_GROUP_PWDG:
        clk_src_group = CLK_SRC_GROUP_PWDG;
        clk_src_index = (EWDG_CTRL0_CLK_SEL_GET(HPM_PWDG->CTRL0) == 0);
        break;
    case CLK_SRC_GROUP_PMIC:
        clk_src_group = CLK_SRC_GROUP_COMMON;
...
hpm_stat_t clock_set_dac_source(clock_name_t clock_name, clk_src_t src)
{
    ...
    if ((src < clk_dac_src_ahb0) || (src > clk_dac_src_ana3)) {
        return status_clk_src_invalid;
    }
...
hpm_stat_t clock_set_source_divider(clock_name_t clock_name, clk_src_t src, uint32_t div)
{
    ...
    case CLK_SRC_GROUP_WDG:
        if (node_or_instance < WDG_INSTANCE_NUM) {
            if (src == clk_wdg_src_ahb0) {
                s_wdgs[node_or_instance]->CTRL0 &= ~EWDG_CTRL0_CLK_SEL_MASK;
    ...
    case CLK_SRC_GROUP_PWDG:
        if (src == clk_pwdg_src_osc24m) {
            HPM_PWDG->CTRL0 &= ~EWDG_CTRL0_CLK_SEL_MASK;
