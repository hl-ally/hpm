...
{
    if (source >= clock_source_dac_clk_end) {
        return status_invalid_argument;
    }
    uint32_t dac_index = (uint32_t) (node - clock_node_dac_start);
    if (dac_index >= ARRAY_SIZE(ptr->DACCLK)) {
        return status_invalid_argument;
    }

    ptr->DACCLK[dac_index] = (ptr->DACCLK[dac_index] & ~SYSCTL_DACCLK_MUX_MASK) | SYSCTL_DACCLK_MUX_SET(source);
