...
void usbh_hid_run(struct usbh_hid *hid_class)
{
    if (hid_class != NULL) {
        if (hid_class->hport->config.intf[hid_class->intf].altsetting[0].intf_desc.bInterfaceProtocol == HID_PROTOCOL_KEYBOARD) {
            usbh_int_urb_fill(&intin_urb[0], hid_class->intin, hid_keyboard_buffer, 8, 0, usbh_hid_keyboard_callback, hid_class);
            usbh_submit_urb(&intin_urb[0]);
            ...
            usbh_int_urb_fill(&intin_urb[1], hid_class->intin, hid_mouse_buffer, 7, 0, usbh_hid_mouse_callback, hid_class);
            usbh_submit_urb(&intin_urb[1]);
