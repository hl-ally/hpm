...
int main(void)
{
    ...
    printf("SEI master BiSS_C sample\n");
    ...
    sei_set_xcvr_rx_point(HPM_SEI, BOARD_SEI_CTRL, sei_get_xcvr_ck0_point(HPM_SEI, BOARD_SEI_CTRL));
    sei_set_xcvr_tx_point(HPM_SEI, BOARD_SEI_CTRL, sei_get_xcvr_ck1_point(HPM_SEI, BOARD_SEI_CTRL));
    ...
    /* [3] sei instructions */
    instr_idx = 0;
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_WAIT, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_0, SEI_DAT_30, 1);  /* ACK:0 */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_WAIT, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_0, SEI_DAT_31, 1);  /* START:1 */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_0, SEI_DAT_0, 1);   /* CDS */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_5, SEI_DAT_2, 12);  /* MT */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_5, SEI_DAT_3, 12);  /* ST */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_5, SEI_DAT_4, 2);   /* Error Warn*/
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_FALL_RISE, SEI_DAT_0, SEI_DAT_5, 6);   /* CRC */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_RECV, SEI_INSTR_M_CK_HIGH, SEI_DAT_0, SEI_DAT_30, 0);       /* timeout */
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_WAIT, SEI_INSTR_M_CK_HIGH, SEI_DAT_0, SEI_DAT_31, 0);
    sei_set_instr(HPM_SEI, instr_idx++, SEI_INSTR_OP_HALT, SEI_INSTR_M_CK_LOW, SEI_DAT_0, SEI_DAT_0, 0);
    ...
    sei_state_transition_config_init(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_0_1, &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true;
    sei_state_transition_config_init(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_1_2, &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true;
    sei_state_transition_config_init(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_2_3, &state_transition_config);
    state_transition_config.disable_clk_check = true;
    state_transition_config.disable_txd_check = true;
    state_transition_config.disable_rxd_check = true;
    state_transition_config.disable_timeout_check = true;
    state_transition_config.disable_instr_ptr_check = true;
    sei_state_transition_config_init(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1, SEI_CTRL_LATCH_TRAN_3_0, &state_transition_config);

    state_transition_latch_config.enable = true;
    state_transition_latch_config.output_select = SEI_CTRL_LATCH_TRAN_0_1;
    state_transition_latch_config.delay = 0;
    sei_state_transition_latch_config_init(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1, &state_transition_latch_config);
    ...
    /* [6] update config*/
    update_config.enable_acc = false;
    update_config.enable_spd = false;
    update_config.enable_pos = true;
    update_config.pos_data_idx = SEI_DAT_3;
    update_config.enable_rev = true;
    update_config.rev_data_idx = SEI_DAT_2;
    update_config.use_override_value = false;
    update_config.update_on_err = false;
    update_config.latch_select = SEI_LATCH_1;
    update_config.data_register_select = BIT2_MASK | BIT3_MASK;    /* SEI_DAT_2, SEI_DAT_3 */
    sei_update_config_init(HPM_SEI, BOARD_SEI_CTRL, &update_config);

    /* [7] interrupt config */
    intc_m_enable_irq_with_priority(BOARD_SEI_IRQn, 1);
    sei_set_irq_enable(HPM_SEI, BOARD_SEI_CTRL, SEI_CTRL_IRQ_INT_FLAG_LATCH1_MASK, true);
    ...
    sei_trigger_input_config_init(HPM_SEI, BOARD_SEI_CTRL, &trigger_input_conifg);
...
void isr_sei(void)
{
    ...
    if (sei_get_irq_status(HPM_SEI, BOARD_SEI_CTRL, SEI_CTRL_IRQ_INT_FLAG_LATCH1_MASK)) {
        sei_clear_irq_flag(HPM_SEI, BOARD_SEI_CTRL, SEI_CTRL_IRQ_INT_FLAG_LATCH1_MASK);
        sample_latch_tm = sei_get_latch_time(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_0);
        update_latch_tm = sei_get_latch_time(HPM_SEI, BOARD_SEI_CTRL, SEI_LATCH_1);
        delta = (update_latch_tm > sample_latch_tm) ? (update_latch_tm - sample_latch_tm) : (update_latch_tm - sample_latch_tm + 0xFFFFFFFFu);
        printf("MT:%#x, ST:%#x, EW:%#x, CRC:%#x, sample_tm:%u, update_tm:%u, TimeDelay:%d*0.1us\n",
                sei_get_data_value(HPM_SEI, SEI_DAT_2),
                sei_get_data_value(HPM_SEI, SEI_DAT_3),
                sei_get_data_value(HPM_SEI, SEI_DAT_4),
                sei_get_data_value(HPM_SEI, SEI_DAT_5),
