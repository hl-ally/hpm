...
struct flashstress_context *flashstress_create(struct flashstress_driver *drv, const char *name)
{
    ...
    if (!drv || !name || !drv->erase_block || !drv->read ||
        ...
        FLASH_LOG("FLASHSTRESS: ERR: drv is invalid!!!\n");
        return ctx;
    }

    if (!drv->erase_chip) {
        FLASH_LOG("[%s]: WARNNING: <erase chip> api invalid!\n", name);
    }
    
    ctx = flashstress_context_alloc();
    if (!ctx) {
        FLASH_LOG("FLASHSTRESS: ERROR: [%s] context alloc failed!!!\n", name);
    }
...
int flashstress_erase_chip(struct flashstress_context *ctx)
{
    ...
    if (drv->erase_chip) {
        ...
        if (drv->erase_chip()) {
            FLASH_LOG("[%s]: ERROR: <erase_chip> FAILED!!!\n");
            return -1;
        }
        erase_time_us = tick_elapesd(start_time_us, interface_get_tick_us());
    } else {
        FLASH_LOG("[%s]: WARNNING: <erase chip> api invalid, <erase block> api to be used!\n", ctx->name);
        offset = 0;
        start_time_us = interface_get_tick_us();
        while (offset < chip_size_bytes) {
            if (drv->erase_block(offset)) {
                FLASH_LOG("[%s]: ERROR: <erase_block> offset: 0x%X FAILED!!!\n", ctx->name, offset);
                return -1;
            }
    ...
    while (offset < chip_size_bytes) {
        ...
        if (drv->read(offset, ctx->data_buf, CONFIG_DATA_BUF_SIZE)) {
            FLASH_LOG("[%s]: ERROR: <erase verify read api> offset: 0x%X FAILED!!!\n", ctx->name, offset);
            return -1;
        }

        for (i = 0; i < CONFIG_DATA_BUF_SIZE; i++) {
            if (ctx->data_buf[i] != erase_value) {
                FLASH_LOG("[%s]: ERROR: <ease verify data> 0x%X!= 0x%X FAILED!!!\n", ctx->name, ctx->data_buf[i], erase_value);
                return -1;
            }
        }
    ...
    FLASH_LOG("[%s]: [erase_chip] PASSED!!!\n", ctx->name);
    FLASH_LOG("[%s]: [erase_chip] take %f seconds!!!\n", ctx->name, erase_time_s);
...
int flashstress_erase_all_blocks(struct flashstress_context *ctx)
{
    ...
    while (offset < chip_size_bytes) {
        ...
        for (i = 0; i < CONFIG_DATA_BUF_SIZE; i++) {
            if (ctx->data_buf[i] != erase_value) {
                FLASH_LOG("[%s]: ERROR: <ease verify data> offset: 0x%X FAILED!!!\n", ctx->name, offset);
                return -1;
            }
        }
    ...
    FLASH_LOG("[%s]: [erase all blocks] PASSED\n", ctx->name);
    FLASH_LOG("[%s]: [erase all blocks] take time: %f seconds\n", ctx->name, erase_time_s);
...
int flashstress_write_read(struct flashstress_context *ctx, uint8_t verify_data)
{
    ...
    while (offset < chip_size_bytes) {
        if (drv->write(offset, ctx->data_buf, write_len)) {
            FLASH_LOG("[%s]: ERROR: <write> offset: 0x%X FAILED!!!\n", ctx->name, offset);
            return -1;
        }
        offset += write_len;
    }
    ...
    while (offset < chip_size_bytes) {
        ...
        if (drv->read(offset, ctx->data_buf, CONFIG_DATA_BUF_SIZE)) {
            FLASH_LOG("[%s]: ERROR: <write verify read api> offset: 0x%X FAILED!!!\n", ctx->name, offset);
            return -1;
        }

        for (i = 0; i < CONFIG_DATA_BUF_SIZE; i++) {
            if (ctx->data_buf[i] != verify_data) {
                FLASH_LOG("[%s]: ERROR: <write verify data> offset: 0x%X FAILED!!!\n", ctx->name, offset);
                return -1;
            }
        }
    ...
    FLASH_LOG("[%s]: [write] PASSED!!!\n", ctx->name);
    FLASH_LOG("[%s]: [write] take time: %f, speed: %f MiB/s\n", ctx->name, write_time_s, write_speed);
    ...
    while (offset < chip_size_bytes) {
        if (drv->read(offset, ctx->data_buf, CONFIG_DATA_BUF_SIZE)) {
            FLASH_LOG("[%s]: ERROR: <read speed read api> offset: 0x%X FAILED!!!\n", ctx->name, offset);
            return -1;
        }
        offset += CONFIG_DATA_BUF_SIZE;
    }
    ...
    while (offset < chip_size_bytes) {
        memset(ctx->data_buf, 0xff, CONFIG_DATA_BUF_SIZE);
        if (drv->read(offset, ctx->data_buf, CONFIG_DATA_BUF_SIZE)) {
            FLASH_LOG("[%s]: ERROR: <read verify read api> offset: 0x%X FAILED!!!\n", ctx->name, offset);
            return -1;
        }

        for (i = 0; i < CONFIG_DATA_BUF_SIZE; i++) {
            if (ctx->data_buf[i] != verify_data) {
                FLASH_LOG("[%s]: ERROR: <read verify data> offset: 0x%X FAILED!!!\n", ctx->name, offset);
                return -1;
            }
        }
    ...
    FLASH_LOG("[%s]: [read] PASSED!!!\n", ctx->name);
    FLASH_LOG("[%s]: [read] take time: %f, speed: %f MiB/s\n", ctx->name, read_time_s, read_speed);
