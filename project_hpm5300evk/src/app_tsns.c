...
void calibrate_tsns(void)
{
    ...
    while(1)
    {
        ...
        if (status_success != otp_write_shadow_register(OTP_SHADOW_TSNS, 
    ...
    printf("after calibration: temp %.1f, read from shadow 0x%x\n", 
                    tsns_get_current_temp(TEST_TSNS),
...
void show_temperature(void)
{
    printf("current temperature %.1f (0x%x)\n",
            tsns_get_current_temp(TEST_TSNS),
            tsns_get_current_temp_in_raw(TEST_TSNS));
}

void tsns_test_init(void)
{
